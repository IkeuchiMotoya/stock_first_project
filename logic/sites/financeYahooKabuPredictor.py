...
def fetch_yahoo_financials(ticker: str, stock_name: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        df = df.drop(columns=["会計方式", "財務数値更新日"])

        df.insert(0, "銘柄名", stock_name)
        df.insert(0, "銘柄コード", ticker)

        df["売上高（百万円）"] = pd.to_numeric(df["売上高（百万円）"], errors="coerce")
        df["純利益（百万円）"] = pd.to_numeric(df["純利益（百万円）"], errors="coerce")
        df["純利益率"] = df["純利益（百万円）"] / df["売上高（百万円）"]
        df["純利益率"] = (df["純利益率"] * 100).round(2).astype(str) + "%"

        return df[[
            "銘柄コード", "銘柄名", "決算期", "売上高（百万円）", "売上総利益（百万円）", "粗利率",
            "営業利益（百万円）", "営業利益率", "経常利益（百万円）", "経常利益率",
            "純利益（百万円）", "純利益率"
        ]]
...
if __name__ == "__main__":
    ...
    for _, row in df.iterrows():
        ticker = row["銘柄コード"]
        stock_name = row["銘柄名"]
        result_df = fetch_yahoo_financials(ticker, stock_name)
